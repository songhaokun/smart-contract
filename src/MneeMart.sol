...
contract MneeMart is Ownable, ReentrancyGuard {
    ...
    function listProduct(string memory _cid, uint256 _price, string memory _name) external returns (uint256) {
        ...
        products[productCounter] = Product({
            id: productCounter, seller: msg.sender, cid: _cid, price: _price, name: _name, active: true, salesCount: 0
        });
    ...
    function purchaseProduct(uint256 _productId) external nonReentrant {
        ...
        mneeToken.safeTransferFrom(msg.sender, address(this), product.price);
    ...
    function getProductCID(uint256 _productId) external view returns (string memory) {
        Product storage product = products[_productId];

        if (product.id == 0) revert ProductDoesNotExist(_productId);

        if (!hasPurchased[msg.sender][_productId] && product.seller != msg.sender) {
            revert AccessDenied();
        }
    ...
    function getProduct(uint256 _productId)
        external
        view
        returns (uint256 id, address seller, uint256 price, string memory name, bool active, uint256 salesCount)
    {
        Product memory product = products[_productId];
        return (product.id, product.seller, product.price, product.name, product.active, product.salesCount);
    }
}
